...
class MessageFavoriteDAO:
    ...
    async def create_favorite(
        ...
        sql = (
            ...
            "(favorite_id, session_id, role, content, `timestamp`, agent_ids, "
            "user_id, success, tokens, message_pair_id, citations, bocha_sum) "
            "SELECT %s, m.session_id, m.role, m.content, m.`timestamp`, "
        ...
        args = [favorite_id, user_id] + list(message_pair_ids)
    ...
    async def list_favorites(self, user_id: str) -> List[dict]:
        ...
        另含 favorite_id 与 session_id（跨会话收藏时前端可区分来源）。
        """
        async with self.pool.acquire() as conn:
            async with conn.cursor(aiomysql.DictCursor) as cur:
                await cur.execute(
                    "SELECT favorite_id, session_id, role, content, timestamp, "
                    "agent_ids, user_id, success, tokens, message_pair_id, "
                    "citations, bocha_sum FROM message_favorites "
