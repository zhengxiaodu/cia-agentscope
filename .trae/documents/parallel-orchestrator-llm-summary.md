...
并行编排（ParallelOrchestrator）多智能体执行结束后的 `summary` 事件目前只是机械拼接（`已为您完成 N 项任务：\n{输出1}\n\n---\n\n{输出2}`，见 [parallel.py:141-152](file:///workspace/app/orchestrator/parallel.py#L141-L152)），没有 LLM 参与整合。本次改为：当并行执行出 **多于 1 个** 任务结果时，**一律**调用一次轻量 LLM（`chat_complete` 非流式调用，**默认业务大模型 models.default**）将各任务结果整合为一份连贯的最终回答；LLM 失败/超时仅作为异常兜底回退现有拼接格式（不设开关）。单结果、全失败场景行为不变。
...
构造函数追加 3 个可选参数（全部有默认值，保证既有调用/测试不破）：
...
- 触发条件：结果数 > 1（调用方已保证）且 `summary_client`/`summary_model_config` 非空；client 为空（仅直接构造/测试场景）直接返回 `None` 走拼接
- prompt 构造（模块级常量）：
...
- import 变更：`from openai import AsyncOpenAI`、`from app.intent.llm_client import chat_complete`、`from app.utils.trace_names import TraceName`
...
（`summary_timeout` 用构造默认 60s，不进配置）

### 3. `app/utils/trace_names.py`
...
（`config/intent_config.yml` **不改动**：无开关，一律总结）
...
- `summary_client=None`（直接构造场景）：不调用 LLM，直接拼接
...
- **一律总结，无开关**（用户已确认）：多结果时必走 LLM 汇总，`intent_config.yml` 不加任何配置；失败/超时回退拼接仅为异常兜底，非可配置行为；汇总超时用构造默认 60s（模块内常量语义）
...
- Langfuse 未启用/模型配置缺失时调用失败自然走回退，无需专门处理
...
4. 人工核对：LLM 异常路径产出回退拼接文案；`intent_config.yml` 无新增条目
