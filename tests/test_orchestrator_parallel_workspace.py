...
适配说明（相对实施计划原始用例）：
- 桩返回值改为与真实方法一致的元组，避免误入降级路径；
- run() 保留 workspace_get started 首条阶段事件（前后端契约），
  agent_id 分支断言相应放宽为"首条 stage_status、末条 direct_done"；
- 取消断言的 sleep(0) 改为 sleep(0.05)，等待取消真正传播完成。
"""
import asyncio
import json
...
async def test_generator_closed_early_cancels_workspace_task():
    ...
    await gen.aclose()
    # 给事件循环一点时间让取消真正传播到 task（远小于 ws_delay=5s）
...
async def test_agent_id_path_awaits_workspace_immediately():
    ...
    types = [e["type"] for e in events]
    # 首条是 workspace_get started 阶段事件（前后端契约），其后进入单 agent 路径
    assert types[0] == "stage_status"
    assert types[-1] == "direct_done"
