...
def test_migrate_fixes_permission_mode_bypass():
    ...
    fixed = migrate_legacy_enum_values(state_dict)
...
def test_migrate_handles_non_dict():
    """非 dict 输入原样返回。"""
    assert migrate_legacy_enum_values(None) is None
    assert migrate_legacy_enum_values("str") == "str"
...
async def test_load_returns_none_when_no_session_service():
    ...
    assert await load_agent_state(None, "s1", "a1") is None


@pytest.mark.asyncio
async def test_load_returns_none_when_no_session_id():

    assert await load_agent_state(MagicMock(), None, "a1") is None
...
async def test_load_returns_none_when_dao_returns_none():
    ...
    assert await load_agent_state(session_service, "s1", "a1") is None
...
async def test_load_full_validate_success():
    ...
    loaded = await load_agent_state(session_service, "s1", "a1")
...
async def test_load_logs_warning_on_validate_failure(caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="app.services.agent_state_store"):
        result = await load_agent_state(session_service, "s1", "a1")
